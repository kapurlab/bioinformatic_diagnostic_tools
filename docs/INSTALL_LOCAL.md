...
bin/kapurtools list                    # see available tools
bin/kapurtools install irma_gui        # clone + build env + build frontend
                                       # (delegates to the tool's deploy/install.sh)
```

The installer clones the tool (pinned version from `tools.yml`) into
`~/.local/share/kapurlab-tools/checkouts/<tool>/`, builds its conda env and
frontend, then launches it and opens your browser.

Re-launch later without rebuilding:

```bash
bin/kapurtools local irma_gui
bin/kapurtools local irma_gui --port 8765    # pin a port if you prefer
```

Check status / update:

```bash
bin/kapurtools status
bin/kapurtools check-updates
bin/kapurtools update irma_gui
```
...
| Item | Path |
|---|---|
| Tool checkout | `~/.local/share/kapurlab-tools/checkouts/<tool>/` |
| Conda env | `<checkout>/env/` (or a named env, per the tool's installer) |
| Built frontend | `<checkout>/frontend/dist/` |

Override the checkout location with `--prefix DIR` or `KAPURTOOLS_HOME`.
If you already have the tools cloned elsewhere (e.g. a shared
`/srv/<lab>/tools` tree), point `KAPURTOOLS_TOOLSDIR` at it and the CLI will use
