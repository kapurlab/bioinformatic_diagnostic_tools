...
#
# Python bytecode is the same defect one layer down, and worse: frontend/dist
# needs a BUILD to go dirty, while __pycache__ needs only for someone to RUN the
# tool once — the interpreter rewrites it on import. irma_gui and genoflu_gui
# have .pyc files committed upstream, so every launch made them permanently
# un-updatable, on every platform, for every user (live 2026-09-15: `update all`
# refused both with "has local source changes:
# backend/app/__pycache__/__init__.cpython-310.pyc"). Nothing is lost by
# tolerating them: bytecode is derived from the .py beside it, so a real source
# edit still shows up as that .py and still blocks. Committing .pyc is an
# upstream hygiene bug too, but that fix cannot reach a checkout that already
# has them tracked — an ignore rule does not apply to tracked files.
tool_blocking_edits() {
  local dir="$1" p
  { git -C "${dir}" diff --name-only 2>/dev/null
    ...
    case "${p}" in
      ...
      # Regenerated by the interpreter on import — see the note above.
...
# --fresh — set a tool's env aside and build from nothing, putting it back if the
# build fails. Written for install-local.sh; shared here since 2026-09-02 because
# install-server.sh needs the same thing: doctor's own repair for an env holding a
# sibling tool's stale packages is `install <tool> --fresh`, and on a site install
# that flag was refused as unknown (ICAR-NIVEDI). Callers set TOOL and FRESH, arm
# `trap restore_env_from_fresh EXIT` (or call it from their own trap) around the
# build, and remove FRESH_ASIDE themselves once the new env passes its self-check.
