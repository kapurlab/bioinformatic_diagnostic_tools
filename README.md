...
├── bin/kapurtools     the CLI (install | local | status | check-updates | update)
...
bin/kapurtools list                 # what's in the suite
bin/kapurtools install irma_gui     # local mode (default) — Linux / macOS / WSL2
bin/kapurtools local irma_gui       # re-launch a tool you've installed
```
...
|---|---|---|
| **Personal Linux / macOS / Windows (WSL2)** | `kapurtools install --local <tool>` | Standalone: conda env + uvicorn + browser at `localhost`. No OOD. See [docs/INSTALL_LOCAL.md](docs/INSTALL_LOCAL.md). |
| **Institutional HPC OOD — as a user** | `kapurtools install --sandbox <tool>` | Per-user app in `~/ondemand/dev/`, no sysadmin needed. See [docs/INSTALL_HPC_OOD.md](docs/INSTALL_HPC_OOD.md). |
| **Institutional HPC OOD — as a sysadmin** | `kapurtools install --server <tool>` | System app under `/var/www/ood/apps/sys`. See [docs/SYSADMIN.md](docs/SYSADMIN.md). |
| **Bare-metal Linux lab server** | `kapurtools install --server all` (+ ood-core) | Full stack incl. OOD core. See [docs/INSTALL_BARE_METAL.md](docs/INSTALL_BARE_METAL.md). |
...
```bash
kapurtools check-updates          # report newer upstream versions
kapurtools update <tool|all>      # move to the newest tag + rebuild
```
