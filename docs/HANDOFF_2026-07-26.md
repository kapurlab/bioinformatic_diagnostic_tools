# ARCHIVED handoff — Results pane rollout, kSNP4 + AMR fixes, site-path removal

Sessions: 2026-07-25 → 27. Owner: tks5563. (Development record, kept in-repo.)
Superseded by [../HANDOFF.md](../HANDOFF.md) (2026-07-27).

> **Archived, not deleted.** §3b (the AMRFinderPlus database decision), §4
> (hard-coded paths) and §5 (not-done items) are still open and are carried forward
> in the current handoff. §7 is this repo's most detailed record of the macOS kSNP4
> saga — the current handoff summarises it and points back here for the specifics.
